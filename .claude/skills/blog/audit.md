...
| GFM 구조 | `node script/validate-post-html.mjs <slug>` 실행 (JP 순수 GFM sanity, jp-site-config §3 — KR Gutenberg `wp:block` 검사 폐기). 미닫힌 코드펜스(```` ``` ```` 홀수) + 깨진 GFM 표(헤더 행 다음 `|---|` 구분행 없음) + 깨진 마크다운 이미지/링크(`![..](`·`[..](` 미닫힘). **하나라도 위반 시 −20** (렌더 깨짐 위험) |
